...
date_sub($currentmonth,date_interval_create_from_date_string("3 months"));
$previous_month=date_format($currentmonth,"Y-m");

$sql_previous = "SELECT * FROM current_bill WHERE user_id='" . $uid . "' AND month = '$previous_month'";
$records_bill = mysqli_query($link, $sql_bill);
$previousmonthbill = mysqli_fetch_assoc($records_bill);
...
$meter = $charge = $total = $month = "";
$meter_err = $charge_err = $total_err = "";
...
$previousmeter=$previousmonthbill['meter'];
    ...
    $sql = "INSERT INTO current_bill (user_id, month, meter, units, charge, total, due) VALUES ('$user_id', '$month', '$units', '$units', '$charge', '$total', '$due')";
